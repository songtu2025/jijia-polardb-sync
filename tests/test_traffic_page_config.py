...
class TrafficPageConfigTest(unittest.TestCase):
    def setUp(self):
        self.apis = {api["api_code"]: api for api in load_api_configs("config/api_config.example.yaml")}

    def test_traffic_page_uses_date_window_and_stays_disabled(self):
        self.assertIn("traffic_page", self.apis)
        api = self.apis["traffic_page"]

        self.assertFalse(api["enabled"])
        self.assertEqual(api["method"], "POST")
        self.assertEqual(api["path"], "/operation/sts/traffic/page")
        ...
        self.assertEqual(api["page"]["max_pages"], 2)

...
class StorageLedgerMonthPageConfigTest(unittest.TestCase):
    def setUp(self):
        self.apis = {api["api_code"]: api for api in load_api_configs("config/api_config.example.yaml")}

    def test_storage_ledger_month_page_uses_full_month_window_and_is_enabled(self):
        self.assertIn("storage_ledger_month_page", self.apis)
        api = self.apis["storage_ledger_month_page"]

        self.assertTrue(api["enabled"])
        self.assertEqual(api["method"], "POST")
        self.assertEqual(api["path"], "/fulfillment/inventory/storageLedgerMonth/page")
        ...
        self.assertEqual(api["page"]["max_pages"], 70)
        ...
        self.assertEqual(api["params"]["monthList"], ["2026-06"])

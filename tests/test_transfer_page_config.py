...
class TransferPageConfigTest(unittest.TestCase):
    def setUp(self):
        self.apis = {api["api_code"]: api for api in load_api_configs("config/api_config.example.yaml")}

    def test_transfer_page_uses_full_window_and_is_enabled(self):
        self.assertIn("transfer_page", self.apis)
        api = self.apis["transfer_page"]

        self.assertTrue(api["enabled"])
        self.assertEqual(api["method"], "POST")
        self.assertEqual(api["path"], "/fulfillment/inventory/transfer/page")
        ...
        self.assertEqual(api["page"]["max_pages"], 100)

...
class InventoryEventPageConfigTest(unittest.TestCase):
    def setUp(self):
        self.apis = {api["api_code"]: api for api in load_api_configs("config/api_config.example.yaml")}

    def test_inventory_event_page_is_limited_and_stays_disabled(self):
        self.assertIn("inventory_event_page", self.apis)
        api = self.apis["inventory_event_page"]

        self.assertFalse(api["enabled"])
        self.assertEqual(api["method"], "POST")
        self.assertEqual(api["path"], "/purchase/store/inventoryEvent/page")

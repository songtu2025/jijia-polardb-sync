...
class StorageLedgerDetailPageConfigTest(unittest.TestCase):
    def setUp(self):
        self.apis = {api["api_code"]: api for api in load_api_configs("config/api_config.example.yaml")}

    def test_storage_ledger_detail_page_uses_full_nested_date_window_and_is_enabled(self):
        self.assertIn("storage_ledger_detail_page", self.apis)
        api = self.apis["storage_ledger_detail_page"]

        self.assertTrue(api["enabled"])
        self.assertEqual(api["method"], "POST")
        self.assertEqual(api["path"], "/purchase/inventory/storageLedgerDetail/page")
        ...
        self.assertEqual(api["page"]["max_pages"], 320)
        ...
        self.assertEqual(api["date_window"]["start_field"], "model.beginReportDate")
        self.assertEqual(api["date_window"]["end_field"], "model.endReportDate")

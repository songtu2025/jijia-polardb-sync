...
class AmazonMskuPageConfigTest(unittest.TestCase):
    def setUp(self):
        self.apis = {api["api_code"]: api for api in load_api_configs("config/api_config.example.yaml")}

    def test_amazon_msku_page_uses_full_window_and_is_enabled(self):
        self.assertIn("amazon_msku_page", self.apis)
        api = self.apis["amazon_msku_page"]

        self.assertTrue(api["enabled"])
        self.assertEqual(api["method"], "POST")
        self.assertEqual(api["path"], "/purchase/goods/amazonMsku/page")
        ...
        self.assertEqual(api["page"]["max_pages"], 200)

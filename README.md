...
另有一批已完成小窗口或空结果验证但默认未启用的接口，例如 `purchase_plan_page`、`fba_inventory_page`、`inventory_event_page`、`inventory_age_page`、`transfer_page`、`lot_no_page` 和若干库存、SKU 映射、详情类接口；这些接口需先评估数据量和业务风险，再决定是否进入每天的 enabled 批量同步。

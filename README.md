...
当前 enabled 批量属于长任务，最近一次 33 个接口完整同步耗时约 3852 秒。ECS 上的 cron 窗口应避免和其他重写入任务重叠。
...
当前 enabled 清单与“当前支持哪些真实积加 API？”一致，共 33 个。各接口的文档 id、路径、分页和执行分层以 `config/api_config.example.yaml` 与 `config/jijia_api_catalog.generated.json` 为准。

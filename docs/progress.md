...
阶段 6T 已完成。已将 `traffic_sku_page` 从完整窗口 disabled 候选提升到 daily enabled，并用完整 `--sync-enabled` 验证 25 个 enabled API 同批次成功；当前真实配置 API 为 50 个，enabled API 为 25 个。
...
- `purchase_plan_page` 当前总量为 0 条；`storage_return_page` 当前总量为 1 条；`strategy_template_page` 当前总量为 19 条；`traffic_analysis_page` 在 `2026-07-02` 单日 CNY 窗口总量为 528 条且限流严格；`traffic_page` 在 `2026-07-02` 单日 CNY/day 窗口总量为 583 条，已用 `pagesize=500`、2 页和页间 65 秒等待完成完整窗口验证；`traffic_sku_page` 在 `2026-07-02` 单日 CNY/day 窗口总量为 170 条，已进入 enabled，`2026-07-03` 窗口当前返回 0 条并已推进 checkpoint；`shipment_data_page` 在 `2026-07-02` 单日窗口总量为 943 条，在 `2026-07-03` 单日窗口总量为 58 条；`storage_ledger_page` 在 `2026-07-02` 单日窗口当前总量为 1163 条，已用 `pagesize=500`、3 页和页间 65 秒等待完成完整窗口验证；`storage_ledger_detail_page` 在 `2026-07-02` 单日窗口总量为 27104 条；`storage_ledger_month_page` 在 `2026-06` 月窗口总量为 6044 条；`inventory_receipts_page` 在 `2026-07-02` 单日窗口总量为 735 条；`purchase_sale_storage_fba_page` 当前 MSKU 数量维度总量为 58955 条；`platform_msku_page` 当前总量为 1707 条，已进入 enabled；`transfer_page` 当前总量为 6755 条，请求约 68 页；`product_page` 当前总量为 8258 条，请求 83 页；`lot_no_page` 当前总量为 8602 条，请求约 87 页；`amazon_msku_page` 当前总量为 18430 条，请求约 185 页；`fba_inventory_page` 和 `fba_inventory_v2_page` 当前总量均为 30759 条，请求约 308 页；`inventory_adjustments_page` 当前总量为 58239 条，请求约 583 页；`product_inventory_page` 当前总量为 118653 条，请求 1187 页；`storage_inbound_page` 当前总量为 174286 条，请求约 1744 页；`inventory_event_page` 当前总量为 2669068 条，请求约 26691 页；`inventory_age_page` 当前总量为 6597161 条，当前小窗口配置为每页 10 条且单页响应很慢。当前 25 个 enabled API 的真实批量同步为 3074 次请求，6T 实测耗时约 76 分钟，必须按长耗时任务安排 cron 窗口。
...
阶段 6U：继续推进完整拉取。优先评估下一个低风险、体量可控的完整窗口候选，或评估 `traffic_page` 是否具备进入 daily enabled 的条件；多页且需要 65 秒等待的接口进入 enabled 前必须重新测算 cron 窗口。

建议目标：

- 只读读取覆盖矩阵、6K 执行分层、6T `traffic_sku_page` enabled 批次证据、6Q-6S 完整窗口证据、6Q-6S 复盘、6N-6P 复盘和 `platform_msku_page` enabled 批次证据。
...
- `api_config` 与覆盖矩阵显示真实配置 API 或 enabled 数量符合本轮目标；当前基线是真实配置 API 50 个、enabled 25 个。

...
阶段 9L：继续推进完整拉取。优先继续 `lot_no_detail` 历史回填，不启用；目标是把 checkpoint 从 `next_param_offset=806` 再推进一个 200 窗口。
...
- 确认 checkpoint 从 `param_offset=806` 推进到 `next_param_offset=1006`，或如实际上游有变化，按 DB 事实记录。
- 9L 不启用 `lot_no_detail`，除非已先证明历史缺口为 0 并验证缺失主键扫描边界。

...
阶段 6M 已完成。已新增 `storage_ledger_detail_page` 默认 disabled 单日窗口同步，当前真实配置 API 为 47 个，enabled API 仍为 24 个。
...
- 当前已配置真实 API 为 47 个，其中 24 个已加入 enabled，`product_detail`、`market_inventory_query`、`storage_inbound_detail`、`country_province_query`、`transfer_detail`、`lot_no_detail`、`delivery_fee_query`、`amazon_msku_page`、`fba_inventory_page`、`fba_inventory_v2_page`、`inventory_adjustments_page`、`inventory_event_page`、`inventory_age_page`、`traffic_analysis_page`、`shipment_data_page`、`storage_ledger_page`、`storage_ledger_detail_page`、`inventory_receipts_page`、`purchase_sale_storage_fba_page`、`transfer_page`、`lot_no_page`、`purchase_plan_page` 和 `procure_detail` 已完成验证但保持 disabled。
...
阶段 6N：继续基于 6K 执行分层推进覆盖，可优先选择第三个 `needs_param_source` 候选，或开始为日期窗口类 disabled 大表制定分批回填策略。

建议目标：

- 只读读取覆盖矩阵、6K 执行分层、6L `procure_detail` 参数源证据、6M `storage_ledger_detail_page` 日期窗口证据、6H-6J 复盘和 `platform_msku_page` enabled 批次证据。
...
- `api_config` 与覆盖矩阵显示真实配置 API 或 enabled 数量符合本轮目标；当前基线是真实配置 API 47 个、enabled 24 个。

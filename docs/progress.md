...
阶段 6Z 已完成。已将 `shipment_data_page` 从完整窗口 disabled 候选提升到 daily enabled，并用完整 `--sync-enabled` 验证 29 个 enabled API 同批次成功；当前真实配置 API 为 50 个，enabled API 为 29 个。
...
阶段 7A：继续推进完整拉取。优先从剩余已验证 disabled 的日期窗口或低风险参数接口中选择一个候选推进完整窗口或小样本验证。

建议目标：

- 只读读取覆盖矩阵、6K 执行分层、6Z `shipment_data_page` enabled 批次证据、6W-6Y 复盘和当前 29 enabled 批次耗时。
- 优先评估一个剩余 disabled 日期窗口接口是否可以推进完整窗口；候选应避开订单、财务敏感明细、客服文本、物流费用和超大慢接口。
- 如果选择依赖参数接口，先只读查询数据库证明参数来源真实存在，再做默认 disabled 小样本配置和单接口验证。
...
- `api_config` 与覆盖矩阵显示真实配置 API 或 enabled 数量符合本轮目标；当前基线是真实配置 API 50 个、enabled 29 个。

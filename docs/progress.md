...
阶段 5T：继续回到覆盖矩阵，选择下一个低风险接口扩大覆盖；优先选择业务风险可控、体量适中、字段清晰的普通分页、单对象或清晰依赖参数接口，并继续避开写操作、数组编码未知、强限流和高敏感接口。

...
阶段 11K 已完成。`procure_detail` 已继续推进第十一个 100 条窗口，累计覆盖 1103/1153 个采购单号且保持 disabled；当前真实配置 API 为 50 个，enabled API 为 36 个。下一阶段 11L 可继续推进 `procure_detail` 参数窗口。
...
阶段 11L：继续推进完整拉取。`procure_detail` 已完成第十一个 100 条窗口并保持 disabled；下一阶段继续用 100 窗口推进采购订单详情剩余历史覆盖。

建议目标：

- 继续保持 `procure_detail.enabled=false` 和 `param_source.limit=100`，从 checkpoint 的 `next_param_offset=1103` 继续推进剩余采购单号。
- 运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api procure_detail`，预期新增约 50 条，checkpoint 推进到约 `next_param_offset=1153`。
- 核验同批次 `sync_batch`、`sync_api_log`、`raw_api_data`、`sync_checkpoint` 和 `failed_request_log`，确认请求成功、失败为 0、空对象为 0。
- 继续不要启用 `procure_detail`；当前只覆盖 1103/1153 个采购单号，离完整历史覆盖仍有约 50 个缺口。
- 11L 完成后不需要三轮复盘；下一次三轮复盘放在 11M 完成后。

...
阶段 10Z 已完成。`lot_no_page` 已通过完整窗口验证进入 enabled；当前真实配置 API 为 50 个，enabled API 为 36 个。下一阶段 11A 可评估 `procure_detail` 的参数窗口，并在完成后复盘 10Y-11A。
...
阶段 11A：继续推进完整拉取。`lot_no_page` 已进入 enabled，下一阶段可优先评估 `procure_detail` 的参数窗口；完成后复盘 10Y-11A 三轮。

建议目标：

- 先只读确认 `procure_detail` 当前配置、checkpoint、raw 基线，以及完整 `lot_no_page.raw_json.poCode` 去重数量；当前 `lot_no_page` 可提供 1153 个采购单号，`procure_detail` 只有 3 条小样本。

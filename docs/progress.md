...
阶段 6H 已完成。已新增 `inventory_receipts_page` 并用市场日期窗口完成真实小窗口同步，当前真实配置 API 为 44 个，enabled API 为 23 个。
...
阶段 6I：继续推进完整覆盖。可以继续接一个低风险报表/库存日期窗口接口，或评估一个已验证 disabled 接口是否具备进入 enabled 的条件。

建议目标：

- 只读读取覆盖矩阵、6H 结果、6E-6G 复盘、6E 追平跳过策略和 `traffic_analysis_page`/`shipment_data_page`/`storage_ledger_page`/`inventory_receipts_page` 的窗口验证证据。
...
- `api_config` 与覆盖矩阵显示真实配置 API 或 enabled 数量符合本轮目标；当前基线是真实配置 API 44 个、enabled 23 个。

...
阶段 11N 已完成。`procure_detail` 已用请求参数 `poCode` 建立稳定 `source_primary_key`，历史 1153 条 raw 已回填主键，并通过缺失扫描 no-op 与完整 `--sync-enabled` 验证后进入 enabled；当前真实配置 API 为 50 个，enabled API 为 37 个。下一阶段 11O 应继续从剩余 13 个 configured disabled API 中选择下一个低风险候选推进。
...
阶段 11O：继续推进完整拉取。`procure_detail` 已进入 enabled，当前 configured disabled 还剩 13 个；下一阶段应从剩余 disabled API 中选择下一个低风险候选。

建议目标：

- 先只读复核剩余 13 个 configured disabled API：`market_inventory_query`、`storage_inbound_detail`、`delivery_fee_query`、`amazon_msku_page`、`fba_inventory_page`、`fba_inventory_v2_page`、`inventory_adjustments_page`、`inventory_event_page`、`inventory_age_page`、`traffic_analysis_page`、`storage_ledger_detail_page`、`storage_ledger_month_page`、`purchase_sale_storage_fba_page`。

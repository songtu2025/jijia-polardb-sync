...
阶段 6A 已完成。已新增并验证 `inventory_age_page` FBA 库龄列表候选，当前真实配置 API 为 40 个，enabled API 为 23 个。
...
阶段 6B：继续推进完整覆盖。6B-6D 是下一组三轮；优先处理低风险候选或开始设计超大表增量/窗口策略，仍不能直接启用大体量接口。

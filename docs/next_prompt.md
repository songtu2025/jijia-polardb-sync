...
阶段 12A 已完成。下一阶段 12B 继续推进完整拉取。`storage_inbound_detail` 缺失扫描窗口已从 500 提高到 1000，本轮补齐 1000 个缺失入库单详情；当前真实配置 API 为 50 个，enabled API 为 45 个，configured disabled 为 5 个。
...
- `storage_inbound_detail` 当前配置为 `enabled=false`、`param_source.limit=1000`、`auto_advance=true`、`exclude_existing_target=true`，参数来自 `storage_inbound_page.raw_json.code`，响应主键为 `code`。
- 阶段 12A 单接口批次 `sync_20260706_062155_460920` 成功，1000 次请求、1000 条成功计数、失败 0，耗时 767 秒。
- 同批次 `storage_inbound_detail` raw 为 1000 条、1000 个 `source_primary_key`、1000 个不同主键、1000 个 `data_hash`，`data_date` 覆盖 `2023-04-04` 到 `2023-07-27`。
- `storage_inbound_detail` 累计覆盖为 1506/174334 个上游去重 code；还没有达到 enabled 条件。
- DB 配置已确认 `api_config.storage_inbound_detail.enabled=0`、`param_source.limit=1000`、`exclude_existing_target=true`、`auto_advance=true`。
- 阶段 12A dry-run 显示 loaded 45 enabled API config(s)，确认未误启用 `storage_inbound_detail`。
- 阶段 12A 覆盖矩阵显示公开文档 API 185 个，真实配置 API 50 个，enabled 45 个，configured disabled 5 个。
- 12B 完成后需要做 11Z-12B 三轮复盘。

建议目标：

- 优先继续 `storage_inbound_detail` 缺失扫描回填；下一轮可保持 `limit=1000` 再跑一批，或先只读评估是否把窗口提高到 2000。
- 不要直接把 `storage_inbound_detail` 加入 enabled；它当前只覆盖 1506/174334。

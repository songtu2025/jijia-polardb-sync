...
- 当前工作区包含大量未提交修改。必须先核对和保护现有改动，不得重建项目、覆盖用户修改或直接提交。

## 当前唯一主线

1. 收口当前工作区，核对代码、迁移、配置、测试和文档是否形成一致发布基线。
2. 在获准的隔离 MySQL/PolarDB 副本执行只读 preflight、结构迁移和快照/PITR 恢复演练。
3. 明确并执行 legacy `jijia_account_id=0` 数据到真实平台账号的受控归属。
4. 建立逐账号、逐 API 的调度割接清单；启用 Web 策略前先停止对应 legacy 调度。
5. 完成全部接口割接后停用生产 `--sync-enabled` cron，使 Web Scheduler 成为唯一生产定时任务来源。
6. 完成 SMTP、HTTPS Cookie、systemd、Nginx、ECS 和真实 Worker 验收，并观察至少两个完整调度周期。
7. 生产证据完整后再清理不再需要的 legacy 业务同步入口；配置校验、连接检查、只读探测和迁移能力可以保留。
...
- 不要引入 Redis、Celery、Kafka、Docker 或多 Worker，除非真实运行证据证明当前架构不能满足需求并获得用户确认。

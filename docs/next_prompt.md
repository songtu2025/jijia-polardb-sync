...
阶段 8Z 已完成。下一阶段 9A 继续推进完整拉取：优先继续 `transfer_detail` 的 200 条窗口，或选择 `lot_no_detail` 做同等中等窗口验证；9B 完成后需要复盘 8Z-9B。
...
- `transfer_detail` 仍是历史回填任务，不要直接加入 enabled；当前只覆盖 5606/6499，daily 增量边界仍需后续设计。
...
- 8Y 删除本地 `logs/token_cache.json` 后重跑成功，成功批次 `sync_20260704_175750_445975`，请求 200 次，写入 200 条，失败 0。
- 8Z 不改 YAML，继续复用 `transfer_detail.param_source.limit=200`，从 5406 推进到 5606，批次 `sync_20260704_181145_259163`，请求 200 次，写入 200 条，失败 0，耗时 387 秒。
- 8Z 后 `transfer_detail` checkpoint 指向批次 `sync_20260704_181145_259163`，记录 `param_offset=5406`、`param_limit=200`、`next_param_offset=5606`、`item_count=200`、`total_count=200`。
- `transfer_detail` 上游 `storage_inbound_page` 中 `opType=TFOutbound` 的不同调拨单号为 6499 个，当前覆盖 5606/6499，约 86.3%。
- 8W-8Y 复盘结论：三轮最终成功窗口累计推进 600 个调拨单详情；8Y 暴露当前客户端不会在业务接口 401 时自动刷新 token，但清缓存后同窗口重跑可恢复。
- 8Z 已运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs`，同步 52 条 API 配置到 DB。
- 8Z 覆盖矩阵刷新为公开文档 API 185 个，真实配置 API 50 个，enabled 32 个。
- 8Z 已运行 `.\\.venv\\Scripts\\python.exe -m compileall app tests` 并通过。
- 8Z 已运行 `.\\.venv\\Scripts\\python.exe -m unittest discover -s tests -p "test_*.py"`，78 个测试通过。
...
1. 先只读读取覆盖矩阵、7X enabled 批次证据、7Y-8Z `transfer_detail` 中等窗口批次证据、各三轮复盘和当前 32 enabled 批次耗时。
2. 如果继续 `transfer_detail`，不改 YAML，直接复用 checkpoint 的 `next_param_offset=5606` 跑下一批 200 条，并核验 checkpoint 推进到 5806。
3. 如果切换接口，优先选择 `lot_no_detail` 做同样的 200 条窗口验证；切换前必须说明体量、参数来源和风险。
4. 不要把 `transfer_detail` 直接加入 enabled；当前只覆盖 5606/6499，daily 增量边界仍需后续设计。

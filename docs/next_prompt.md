...
阶段 9E 已完成。下一阶段 9F 继续推进完整拉取：优先评估 `transfer_detail` 从历史回填切换到 daily 增量的边界，不要直接加入 enabled。
...
- `transfer_detail` 已完成当前历史回填，但不要直接加入 enabled；当前覆盖 6499/6499，daily 增量边界仍需后续设计。
...
- 9E 不改 YAML，继续复用 `transfer_detail.param_source.limit=200`，从 6406 推进到 6499，批次 `sync_20260704_191625_238364`，请求 93 次，写入 93 条，失败 0，耗时 174 秒。
- 9E 后 `transfer_detail` checkpoint 指向批次 `sync_20260704_191625_238364`，记录 `param_offset=6406`、`param_limit=200`、`next_param_offset=6499`、`item_count=93`、`total_count=93`。
- `transfer_detail` 上游 `storage_inbound_page.raw_json.fcode` 中 `opType=TFOutbound` 的不同调拨单号为 6499 个，当前覆盖 6499/6499，剩余缺口 0。
...
- 9E 已运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs`，同步 52 条 API 配置到 DB。
- 9E 覆盖矩阵刷新为公开文档 API 185 个，真实配置 API 50 个，enabled 32 个。
- 9E dry-run 显示 loaded 32 enabled API config(s)，说明 `transfer_detail` 没有误进入 enabled。
- 9E 已运行 `.\\.venv\\Scripts\\python.exe -m compileall app tests` 并通过。
- 9E 已运行 `.\\.venv\\Scripts\\python.exe -m unittest discover -s tests -p "test_*.py"`，78 个测试通过。
...
1. 先只读读取覆盖矩阵、7X enabled 批次证据、7Y-9E `transfer_detail` 中等窗口批次证据、9C-9E 复盘、本轮全面复盘和当前 32 enabled 批次耗时。
2. 优先评估 `transfer_detail` 是否能复用缺失主键扫描作为 daily 增量边界；如果要改 YAML，应先保持 disabled，单接口验证空缺口或新增缺口行为。
3. 如果切换接口，优先选择 `lot_no_detail` 做同样的 200 条窗口验证；切换前必须说明体量、参数来源和风险。
4. 不要把 `transfer_detail` 直接加入 enabled；当前只是历史回填完成，daily 增量边界仍需后续设计。

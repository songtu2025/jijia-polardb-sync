...
阶段 6P 已完成。下一阶段 6Q 优先从已验证 disabled 接口中选择一个体量可控的候选，评估是否能从“小窗口验证”推进到“完整单接口回填验证”；如果风险过高，再继续按 6K 分层新增默认 disabled 候选。
...
- 6P 覆盖矩阵执行分层摘要为：`configured=50`、`needs_upstream_params=63`、`needs_sensitive_review=22`、`defer_or_review=50`。
- 6K 为 `app.doc_catalog` 增加执行分层字段：`execution_bucket`、`execution_stage`、`execution_reason`。
- 6N 新增 `storage_ledger_month_page`，文档 id 为 `2658`，路径为 `POST /fulfillment/inventory/storageLedgerMonth/page`，默认保持 `enabled=false`，使用 `monthList=["2026-06"]`；真实批次 `sync_20260703_173133_727406` 写入 100 条，总量 6044。
- 6O 新增单层数组参数源能力，支持 `raw_json.marketListVos[].marketId`；同轮新增 `traffic_sku_page`，文档 id 为 `123`，路径为 `POST /operation/sts/trafficSku/page`，默认保持 `enabled=false`；真实批次 `sync_20260703_174553_317496` 写入 100 条，总量 170。
- 6P 新增 `traffic_page`，文档 id 为 `122`，路径为 `POST /operation/sts/traffic/page`，默认保持 `enabled=false`。
- 6P 官方文档确认 `traffic_page` 必填 `currency`、`beginDate`、`endDate`、`page`、`pagesize`、`viewType`；响应 `data.rows`，总数 `data.total`，日期字段 `recordDate`。
- 6P 配置使用 `currency=CNY`、`viewType=day`、`date_window.start_field=beginDate`、`date_window.end_field=endDate`、`default_start=2026-07-02`、`page.max_pages=1`、`primary_key.field=""`。
- 6P 的 `.\\.venv\\Scripts\\python.exe -m app.main` dry-run 显示 24 个 enabled API。
- 6P 的 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs` 已通过，同步配置数 52；数据库总配置 52 条、启用 24 条，`traffic_page.enabled=0`。
- 6P 的 `traffic_page` 正式单接口批次为 `sync_20260703_175618_116241`，状态 `success`，`rows=100`、`requests=1`、失败 0。
- 6P 数据库核验：同批次 `sync_batch` 成功，`sync_api_log` 成功，raw 100 条、100 个 distinct hash、100 条都有 `raw_json` 和 `data_date=2026-07-02`。
- 6P checkpoint 记录 `last_page=1`、`request_count=1`、`item_count=100`、`total_count=583`、`window_start=2026-07-02`、`window_end=2026-07-02`、`next_window_start=2026-07-03`。
- 6N-6P 三轮复盘已写入 `docs/progress.md`；结论是覆盖继续扩大，但 `storage_ledger_month_page`、`traffic_page`、`traffic_sku_page` 都仍是小窗口验证，不能当作完整拉取。
...
- `deliveryFee/query`、`relevancePoInfo/query`、`traffic_analysis_page`、`traffic_page` 和 `traffic_sku_page` 这类统计或费用接口后续应减少手工扫参，优先用小窗口同步和较长等待。
...
1. 先只读读取覆盖矩阵、6K 执行分层、6N/6O/6P 证据、6N-6P 复盘和当前 disabled 已验证接口清单。
2. 优先评估一个已验证 disabled 接口是否可以推进到完整单接口回填验证；候选应避开订单、财务敏感明细、客服文本、物流费用和严格限流大表。
3. 如果评估完整回填，必须先确认 `max_pages`、日期/月窗口、总量、限流、预估请求数和 cron 窗口，不能把接入阶段小窗口误当完整拉取。
4. 如果继续新增 API，仍优先从 `needs_param_source` 中选择能用现有能力证明真实参数来源的接口。
5. 如果候选涉及依赖参数，先只读查询数据库证明参数来源真实存在，再新增默认 disabled 小样本配置。
6. 如果现有 `param_source.fields`、`param_source.filters`、`param_source.auto_advance`、单层数组展开或 `date_window` 不够用，必须测试先行做最小扩展。

...
阶段 12F 已完成。下一阶段 12G 继续推进完整拉取。`storage_inbound_detail` 继续使用 5000 缺失扫描窗口，本轮补齐 5000 个缺失入库单详情；当前真实配置 API 为 50 个，enabled API 为 45 个，configured disabled 为 5 个。
...
- 阶段 12F 首次单接口批次 `sync_20260706_112018_586573` 因 `raw_api_data` upsert 锁等待超时失败；已确认根因是本轮后台启动尝试留下 MySQL Sleep 事务，已结束线程 `3999103` 并确认 `information_schema.innodb_trx` 为空。
- 阶段 12F 重跑单接口批次 `sync_20260706_112231_522900` 成功，5000 次请求、5000 条成功计数、失败 0，耗时 4191 秒。
...
- `storage_inbound_detail` 累计覆盖为 21506/174334 个上游去重 code；还没有达到 enabled 条件。
...
- 阶段 12F dry-run 显示 loaded 45 enabled API config(s)，确认未误启用 `storage_inbound_detail`。
- 阶段 12F 覆盖矩阵显示公开文档 API 185 个，真实配置 API 50 个，enabled 45 个，configured disabled 5 个。
...
- 下一次三轮复盘放在 12H 完成后；12G 是 12F-12H 复盘前的第二轮。
- 如遇 `Lock wait timeout exceeded`，先查 `information_schema.processlist` 和 `information_schema.innodb_trx`，不要直接反复重跑。

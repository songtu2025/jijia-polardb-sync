...
阶段 12J 已完成。`storage_inbound_detail` 在 12I named lock 已生效后继续用 2000 窗口回填，批次 `sync_20260710_215533_340234` 成功补齐 2000 个缺失入库单详情；下一阶段 12K 建议继续 2000 窗口，并完成 12I-12K 三轮复盘。
...
- `storage_inbound_detail` 当前配置为 `enabled=false`、`param_source.limit=2000`、`auto_advance=true`、`exclude_existing_target=true`，累计覆盖为 25506/174334 个上游去重 code，完成约 14.63%，剩余 148828。
...
- 12J 回填结束后 named lock 已释放，`information_schema.innodb_trx=0`；本批次和该 API 累计 `failed_request_log` 均为 0。
...
- 下一次三轮复盘放在 12K 完成后，复盘 12I named lock、12J 回填和 12K 回填。

验收：

- 新接口、完整窗口或回填评估必须由公开文档、覆盖矩阵、真实请求、数据库只读查询或测试证明，不靠猜测字段。
- 如启用接口，必须证明 `api_config.enabled=1`、dry-run enabled 数量变化正确，并用真实同步批次证明成功。
- 如继续 `storage_inbound_detail`，必须证明 `exclude_existing_target=true` 生效、批次没有重复主键、失败日志为 0，并记录累计覆盖进度；当前覆盖基线是 25506/174334。

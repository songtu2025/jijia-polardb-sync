...
阶段 12S 已完成。`storage_inbound_detail` 在 12I named lock 已生效后继续完成 2000 窗口回填，最新批次 `sync_20260711_043506_686038` 成功补齐 2000 个缺失入库单详情；12O-12Q 三轮复盘已完成。下一阶段 12T 建议继续 2000 窗口，并完成 12R-12T 三轮复盘。
...
- `storage_inbound_detail` 当前配置为 `enabled=false`、`param_source.limit=2000`、`auto_advance=true`、`exclude_existing_target=true`，累计覆盖为 43506/174334 个上游去重 code，完成约 24.96%，剩余 130828。
...
- 12S 回填结束后 named lock 已释放，`information_schema.innodb_trx=0`；本批次和该 API 累计 `failed_request_log` 均为 0。

建议目标：

- 优先继续 `storage_inbound_detail` 缺失扫描回填；下一轮建议继续 `limit=2000`，避免当前前台执行窗口再次被 5000 长任务拖住。
- 不要直接把 `storage_inbound_detail` 加入 enabled；它当前只覆盖 43506/174334。
...
- 12O-12Q 三轮复盘已完成：三轮均成功，覆盖从 33506 推进到 39506/174334，净增 6000；耗时从 2069 秒、1730 秒下降到 1600 秒，2000 窗口仍适合当前前台执行方式。12R-12S 已继续补齐 4000 条，下一次三轮复盘放在 12T 完成后。

验收：

- 新接口、完整窗口或回填评估必须由公开文档、覆盖矩阵、真实请求、数据库只读查询或测试证明，不靠猜测字段。
- 如启用接口，必须证明 `api_config.enabled=1`、dry-run enabled 数量变化正确，并用真实同步批次证明成功。
- 如继续 `storage_inbound_detail`，必须证明 `exclude_existing_target=true` 生效、批次没有重复主键、失败日志为 0，并记录累计覆盖进度；当前覆盖基线是 43506/174334。

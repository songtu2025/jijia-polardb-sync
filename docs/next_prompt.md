...
2. 将 `role_list.enabled` 从 `false` 改为 `true`。
3. 运行 `.\\.venv\\Scripts\\python.exe -m app.main`，确认 enabled API 变为 3 个。
4. 运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-enabled`。
5. 查询数据库确认同一个 `sync_batch` 下有 3 条 `sync_api_log`。
6. 运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs`，同步 `api_config` 表。
7. 查询数据库确认 `api_config.role_list.enabled=1`。

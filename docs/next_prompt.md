...
阶段 7F 已完成。下一阶段 7G 继续推进完整拉取：可继续用 `product_detail.limit=500` 从 `next_param_offset=1619` 推进下一段，或选择另一个低风险参数型接口做更大窗口验证。
...
- 7F 继续选择 `product_detail`，复用 `product_detail.param_source.limit=500`，仍保持 `enabled=false`。
- 7F 的真实单接口批次为 `sync_20260704_051615_422492`，`product_detail` 请求 500 次、写入 500 条、失败 0。
- 7F 数据库核验：该批次 `sync_batch.status=success`、`total_api_count=1`、`success_api_count=1`、`failed_api_count=0`；同批次 `sync_api_log.status=success`、`request_count=500`、`success_count=500`、`failed_count=0`。
- 7F 同批次 `raw_api_data` 写入 500 条 `product_detail`，500 个不同 `source_primary_key`，500 个不同 `data_hash`，无缺失主键。
- 7F 后 `product_detail` checkpoint 记录 `param_offset=1119`、`param_limit=500`、`next_param_offset=1619`、`item_count=500`、`total_count=500`。
- `product_detail` 当前累计 raw 覆盖 1619 条，1619 个不同产品主键；上游 `product_page` 当前有 8258 个不同产品主键。
- 7F 后 dry-run 仍显示 30 个 enabled API，说明 `product_detail` 没有误进入 enabled。
- 7F 后覆盖矩阵执行分层摘要仍为：`configured=50`、`needs_upstream_params=63`、`needs_sensitive_review=22`、`defer_or_review=50`。
- 7F 已运行 `.\\.venv\\Scripts\\python.exe -m app.doc_catalog --output config\\jijia_api_catalog.generated.json --summary` 并通过。
- 7F 已运行 `.\\.venv\\Scripts\\python.exe -m compileall app tests` 并通过。
- 7F 已运行 `.\\.venv\\Scripts\\python.exe -m unittest discover -s tests -p "test_*.py"`，76 个测试通过。
- 7C-7E 三轮复盘已完成：`product_detail` 从 100 请求窗口提升到 500 请求窗口，并连续两次 500 请求窗口成功；当前安全回填粒度可定为 500 请求窗口。
- 7F 是 7C-7E 复盘后的第 1 轮，500 请求窗口第三段成功；下一次三轮复盘应在 7H 完成后覆盖 7F-7H。
...
1. 先只读读取覆盖矩阵、7F `product_detail` 批次证据、7E `product_detail` 批次证据和当前 30 enabled 批次耗时。
2. 如果继续推进产品详情，复用 `product_detail.limit=500` 从 `next_param_offset=1619` 开始，不要直接跳到全量。

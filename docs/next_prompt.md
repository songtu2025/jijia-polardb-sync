...
阶段 15H 已完成。`storage_inbound_detail` 在覆盖 174334/174334 后完成空缺口验证，批次 `sync_20260712_191559_497680` 成功，0 请求、0 写入、0 失败。下一阶段 15I 建议做 enabled 主链路边界只读评估，并完成 15G-15I 三轮复盘。
...
- 15H 最终 DB 复核显示累计覆盖 174334/174334、`failed_request_log=0`、named lock 已释放、外部 `information_schema.innodb_trx=0`，DB `api_config` 仍为 59 条、enabled 45 条，`storage_inbound_detail.enabled=0`。
- 15H 后 `sync_checkpoint` 指向批次 `sync_20260712_191559_497680`，`checkpoint_value` 记录 `request_count=0`、`item_count=0`、`total_count=0`、`param_limit=2000`、`next_param_offset=0`。
...
- 15D-15F 三轮复盘已完成：覆盖从 167506/174334 推进到 173506/174334，净增 6000。15G 已作为下一组三轮第 1 轮完成，覆盖从 173506 推进到 174334/174334。15H 已作为第 2 轮完成，空缺口验证为 0 请求、0 写入、0 失败。

建议目标：

- 15I 优先只读评估 `storage_inbound_detail` 是否具备进入 enabled 的最小边界：确认 `--sync-enabled` 会复用同一套 `exclude_existing_target=true` 参数源、空缺口时不会重复请求历史、新增上游 code 能在后续批次被发现。
- 15I 不要直接启用 `storage_inbound_detail`；如要启用，必须先给出最小实现方案并获得确认。
- 15I 完成后做 15G-15I 三轮复盘和整体规划。

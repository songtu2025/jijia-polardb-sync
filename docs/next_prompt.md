...
阶段 9S 已完成。下一阶段 9T 继续推进完整拉取：继续 `lot_no_detail` 历史回填，不启用；目标是把 checkpoint 从 `next_param_offset=2406` 再推进一个 200 窗口到约 2606。9T 完成后需要复盘 9R-9T 三轮。
...
- 9S 起点只读确认 `lot_no_detail` 已覆盖 2206/8261 个交货单详情，剩余缺口 6055。
- 9S 已运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api lot_no_detail`，批次 `sync_20260704_230406_864784` 成功，请求 200 次、写入 200 条、失败 0。
- 9S 同批次 `sync_batch.status=success`、`total_api_count=1`、`success_api_count=1`、`failed_api_count=0`，从 `2026-07-04 23:04:07` 到 `2026-07-04 23:09:01`。
- 9S 同批次 `sync_api_log` 为 `status=success`、`request_count=200`、`success_count=200`、`failed_count=0`、`error_message=NULL`。
- 9S 同批次 raw 为 200 条、200 个不同 `source_primary_key`、200 个不同 `data_hash`；`failed_request_log=0`。
- 9S 后 `lot_no_detail` checkpoint 指向批次 `sync_20260704_230406_864784`，记录 `param_offset=2206`、`param_limit=200`、`next_param_offset=2406`、`item_count=200`、`total_count=200`。
- 9S 后 `lot_no_detail` 累计 raw 为 2406 条、2406 个不同交货单号；按 `storage_inbound_page.raw_json.fcode` 且 `opType=LNInbound` 口径剩余缺口为 5855 个。
- 9S 已运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs`，同步 52 条 API 配置到 DB。
- 9S DB 核验显示 `api_config` 总配置 52 条、enabled 33 条，`lot_no_detail.enabled=0`、`config_json.enabled=false`、`param_source.limit=200`。
- 9S dry-run 显示 loaded 33 enabled API config(s)，说明 `lot_no_detail` 没有误进入 enabled。
- 9S 已运行 `.\\.venv\\Scripts\\python.exe -m app.doc_catalog --output config\\jijia_api_catalog.generated.json --summary`，公开文档 API 185 个、真实配置 API 50 个、enabled 33 个。
- 9S 已运行 `.\\.venv\\Scripts\\python.exe -m compileall app tests` 并通过。
- 9S 已运行 `.\\.venv\\Scripts\\python.exe -m unittest discover -s tests -p "test_*.py"`，80 个测试通过。
...
4. 确认 checkpoint 从 `param_offset=2406` 推进到 `next_param_offset=2606`，或如实际上游有变化，按 DB 事实记录。
5. 9T 不启用 `lot_no_detail`，除非已先证明历史缺口为 0 并验证缺失主键扫描边界。
...
11. 9T 完成后复盘 9R-9T 三轮：累计推进量、失败数、耗时和继续策略。
12. 提交推送时不要提交 `.env`、token 缓存、日志或任何敏感信息。

验收：

1. 新接口、完整窗口、参数窗口或 enabled 评估必须由公开文档、覆盖矩阵、真实请求、数据库只读查询或测试证明，不靠猜测字段。
2. 9T 默认不启用 `lot_no_detail`；如启用新接口，必须先证明历史缺口为 0、`api_config.enabled=1`、dry-run enabled 数量变化正确，并用真实 `--sync-enabled` 批次证明全部 enabled API 同批次成功。

...
阶段 15K 已完成。15K 只读复核了剩余 configured disabled API 的风险分层，没有改代码、没有改 YAML、没有启用接口、没有同步 DB enabled 状态。

当前事实：

- 当前最新提交应为阶段 15K 文档提交；开始前请核对 `git status --short --branch` 和 `git log -1 --oneline`。
...
- `storage_inbound_detail` 当前配置为 `enabled=false`、`param_source.limit=2000`、`auto_advance=true`、`exclude_existing_target=true`，累计覆盖为 174334/174334 个上游去重 code；最新空缺口批次 `sync_20260712_191559_497680` 为 0 请求、0 写入、0 失败。
...
- dry-run 仍显示 45 个 enabled API；15K 终态 DB named lock 空闲且排除当前核验连接后外部 `information_schema.innodb_trx=0`。

建议目标：

- 阶段 15L 优先在获得明确确认后执行一个最小实施项，并在完成后做 15J-15L 三轮复盘。
- 推荐优先项：同步任务互斥锁连接加 `AUTOCOMMIT`。最小范围是改 `app/main.py` 的 `_sync_task_lock()`，让 `engine.connect().execution_options(isolation_level="AUTOCOMMIT")` 成为锁专用连接；补 `tests/test_main_sync_lock.py` 断言；运行锁测试、dry-run、compileall、unittest、diff-check 和 DB 锁/事务复核。
- 备选项：`storage_inbound_detail` enabled 最小实施。需要更新测试期望、将 `storage_inbound_detail.enabled` 改为 `true`、同步 DB `api_config`、dry-run 验证 46 个 enabled、运行真实批次证明成功、做 DB 复核。
...
- 如调整互斥锁，必须证明写入口仍受 named lock 保护，拿不到锁会退出，异常路径释放锁，释放失败不掩盖任务成功，锁连接不留下外部 InnoDB 事务。

...
阶段 8M 已完成。下一阶段 8N 继续推进完整拉取：优先继续 `transfer_detail` 的 200 条窗口，或选择 `lot_no_detail` 做同等中等窗口验证。
...
- 8M 不改 YAML，继续复用 `transfer_detail.param_source.limit=200`。
- 8M dry-run 仍显示 loaded 32 enabled API config(s)，说明 `transfer_detail` 没有误进入 enabled。
- 8M 的真实单接口批次为 `sync_20260704_151048_289646`。
- 8M `transfer_detail` 批次命令输出：请求 200 次，写入 200 条。
- 8M DB 核验：该批次 `sync_batch.status=success`、`total_api_count=1`、`success_api_count=1`、`failed_api_count=0`，从 `2026-07-04 15:10:48` 到 `2026-07-04 15:18:29`，耗时 461 秒。
- 8M 同批次 `sync_api_log` 为 `status=success`、`request_count=200`、`success_count=200`、`failed_count=0`。
- 8M 同批次 `raw_api_data` 写入 200 条，200 个不同 `source_primary_key`，200 个不同 `data_hash`，`data_date` 范围为 `2024-08-07` 到 `2024-09-14`。
- 8M 同批次 `failed_request_log` 为 0 条；样本确认 `source_primary_key` 与 `raw_json.code` 一致。
- 8M 后 `transfer_detail` 当前累计 raw 为 3006 条、3006 个不同调拨单号；checkpoint 指向批次 `sync_20260704_151048_289646`，记录 `param_offset=2806`、`param_limit=200`、`next_param_offset=3006`、`item_count=200`、`total_count=200`。
- `transfer_detail` 上游 `storage_inbound_page` 中 `opType=TFOutbound` 的不同调拨单号为 6499 个，当前覆盖 3006/6499，约 46.3%。
- 8M 已运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs`，同步 52 条 API 配置到 DB。
- 8M 覆盖矩阵刷新为公开文档 API 185 个，真实配置 API 50 个，enabled 32 个。
- 8M 已运行 `.\\.venv\\Scripts\\python.exe -m compileall app tests` 并通过。
- 8M 已运行 `.\\.venv\\Scripts\\python.exe -m unittest discover -s tests -p "test_*.py"`，78 个测试通过。
...
1. 先只读读取覆盖矩阵、7X enabled 批次证据、7Y-8M `transfer_detail` 中等窗口批次证据、8A-8C、8D-8F、8G-8I 与 8K-8M 复盘和当前 32 enabled 批次耗时。
2. 如果继续 `transfer_detail`，不改 YAML，直接复用 checkpoint 的 `next_param_offset=3006` 跑下一批 200 条，并核验 checkpoint 推进到 3206。
3. 如果切换接口，优先选择 `lot_no_detail` 做同样的 200 条窗口验证；切换前必须说明体量、参数来源和风险。
4. 不要把 `transfer_detail` 直接加入 enabled；当前只覆盖 3006/6499，daily 增量边界仍需后续设计。

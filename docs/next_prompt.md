...
阶段 10I 已完成。下一阶段 10J 继续推进完整拉取：继续 `lot_no_detail` 历史回填，不启用；目标是把 checkpoint 从 `next_param_offset=5606` 再推进一个 200 窗口到约 5806。下一次三轮复盘放在 10L 完成后。
...
4. 确认 checkpoint 从 `param_offset=5606` 推进到 `next_param_offset=5806`，或如实际上游有变化，按 DB 事实记录。
5. 10J 不启用 `lot_no_detail`，除非已先证明历史缺口为 0 并验证缺失主键扫描边界。
...
2. 10J 默认不启用 `lot_no_detail`；如启用新接口，必须先证明历史缺口为 0、`api_config.enabled=1`、dry-run enabled 数量变化正确，并用真实 `--sync-enabled` 批次证明全部 enabled API 同批次成功。

...
阶段 9C 已完成。下一阶段 9D 继续推进完整拉取：优先继续 `transfer_detail` 的 200 条窗口，或选择 `lot_no_detail` 做同等中等窗口验证；9C-9E 三轮完成后需要下一次复盘。
...
- `transfer_detail` 仍是历史回填任务，不要直接加入 enabled；当前只覆盖 6206/6499，daily 增量边界仍需后续设计。
...
- 9C 后 `transfer_detail` checkpoint 指向批次 `sync_20260704_184937_597382`，记录 `param_offset=6006`、`param_limit=200`、`next_param_offset=6206`、`item_count=200`、`total_count=200`。
- `transfer_detail` 上游 `storage_inbound_page` 中 `opType=TFOutbound` 的不同调拨单号为 6499 个，当前覆盖 6206/6499，约 95.5%。
...
- 9C 已运行 `.\\.venv\\Scripts\\python.exe -m app.main --sync-api-configs`，同步 52 条 API 配置到 DB。
- 9C 覆盖矩阵刷新为公开文档 API 185 个，真实配置 API 50 个，enabled 32 个。
- 9C 已运行 `.\\.venv\\Scripts\\python.exe -m compileall app tests` 并通过。
- 9C 已运行 `.\\.venv\\Scripts\\python.exe -m unittest discover -s tests -p "test_*.py"`，78 个测试通过。
...
1. 先只读读取覆盖矩阵、7X enabled 批次证据、7Y-9C `transfer_detail` 中等窗口批次证据、各三轮复盘和当前 32 enabled 批次耗时。
2. 如果继续 `transfer_detail`，不改 YAML，直接复用 checkpoint 的 `next_param_offset=6206` 跑下一批 200 条，并核验 checkpoint 推进到 6406。
3. 如果切换接口，优先选择 `lot_no_detail` 做同样的 200 条窗口验证；切换前必须说明体量、参数来源和风险。
4. 不要把 `transfer_detail` 直接加入 enabled；当前只覆盖 6206/6499，daily 增量边界仍需后续设计。
